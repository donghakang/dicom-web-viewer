...
class CornerstoneViewport extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    //#region Change WW, WC, zoom by props -  Dongha Kang
    // value of wc, ww, zoom will be changed with interface in Cornerstone Viewport
    const wcChanged = this.props.wc !== prevProps.wc;
    const wwChanged = this.props.ww !== prevProps.ww;
    const scaleChanged = this.props.scale !== prevProps.scale;

    //WW/WC/Zoom change
    if (wcChanged) {
      const viewport = cornerstone.getViewport(this.element);
      viewport.voi.windowCenter = this.props.wc;
      cornerstone.setViewport(this.element, viewport);
      this.setState({
        windowCenter: this.props.wc,
      });
    }
    if (wwChanged) {
      const viewport = cornerstone.getViewport(this.element);
      viewport.voi.windowWidth = this.props.ww;
      cornerstone.setViewport(this.element, viewport);
      this.setState({
        windowWidth: this.props.ww,
      });
    }
    if (scaleChanged) {
      const viewport = cornerstone.getViewport(this.element);
      viewport.scale = this.props.scale;
      cornerstone.setViewport(this.element, viewport);
      this.setState({
